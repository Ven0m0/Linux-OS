...
systemctl is-enabled "$unit" &>/dev/null || systemctl enable "$unit"
```

## Lock File Pattern (pacman DB)

```bash
[[ -f /var/lib/pacman/db.lck ]] && {
  warn "stale pacman lock — removing"
...
has fwupdmgr && {
  ...
  fwupdmgr update --no-reboot-check 2>/dev/null || true
}
```
