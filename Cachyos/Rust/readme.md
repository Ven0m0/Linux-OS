...
```bash
export RUSTFLAGS="-Copt-level=3 -Ctarget-cpu=native -Ccodegen-units=1 -Cstrip=symbols -Clto=fat -Cembed-bitcode -Zunstable-options -Zdylib-lto -Ztune-cpu=native -Cpanic=abort -Zthreads=8 -Cllvm-args=-enable-dfa-jump-thread -Zfewer-names -Zmerge-functions=aliases -Zprecise-enum-drop-elaboration=yes"
```
