...
detect_adb(){
  if ((IS_TERMUX)); then
    local adb_cmd="$(hasname rish)" || { warn "rish not found; install Shizuku for device access"; return 1; }
  else
    local adb_cmd="$(hasname adb)" || { err "adb not found; install Android SDK platform-tools"; return 1; }
  fi
  printf '%s' "$adb_cmd"
}
ash(){
...
task_aggressive_cleanup(){
  ...
  log "Running cleanup (batched single adb shell)..."
  ash <<'CLEANUP_EOF' && msg "Cleanup complete" || err "Cleanup failed"
...
settings put global dynamic_power_savings_enabled 1
settings put global adaptive_battery_management_enabled 0
settings put global battery_saver_constants "vibration_disabled=true,animation_disabled=true,soundtrigger_disabled=true,fullbackup_deferred=true,keyvaluebackup_deferred=true,gps_mode=low_power,data_sa[...]
...
7) Aggressive cleanup + Experimental batch
8) Experimental batch only
[Termux]
9) Full Termux optimize
10) Package maintenance
11) Cache cleanup
12) Filesystem hygiene
13) Find large files [MB] [path]
u) Update locate DB
[Other]
