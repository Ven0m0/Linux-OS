...
detect_pkg_manager(){
  if [[ -n $_PKG_MGR_CACHED ]];then printf '%s\n' "$_PKG_MGR_CACHED" "${_AUR_OPTS_CACHED[@]}";return 0;fi
  local pkgmgr
  if has paru;then pkgmgr=paru;_AUR_OPTS_CACHED=(--batchinstall --combinedupgrade --nokeepsrc)
  elif has yay;then pkgmgr=yay;_AUR_OPTS_CACHED=(--answerclean y --answerdiff n --answeredit n --answerupgrade y)
  else pkgmgr=pacman;_AUR_OPTS_CACHED=();fi
  _PKG_MGR_CACHED=$pkgmgr
  printf '%s\n' "$pkgmgr" "${_AUR_OPTS_CACHED[@]}"
}
get_pkg_manager(){ [[ -z $_PKG_MGR_CACHED ]] && detect_pkg_manager >/dev/null;printf '%s\n' "$_PKG_MGR_CACHED";}
get_aur_opts(){ [[ -z $_PKG_MGR_CACHED ]] && detect_pkg_manager >/dev/null;printf '%s\n' "${_AUR_OPTS_CACHED[@]}";}
vacuum_sqlite(){
  local db=$1 s_old s_new
  [[ -f $db ]]||{ printf '0\n';return;}
  [[ -f ${db}-wal||-f ${db}-journal ]]&&{ printf '0\n';return;}
  head -c 16 "$db" 2>/dev/null|grep -qF -- 'SQLite format 3'||{ printf '0\n';return;}
...
ensure_not_running_any(){
  ...
  while ((wait_time-->0));do pgrep -x -u "$USER" -f "$pattern" &>/dev/null||return;sleep 1;done
...
mozilla_profiles(){
  ...
  [[ -d $base ]]||return 0
  if [[ -f $base/installs.ini ]];then while IFS= read -r p;do [[ -d $base/$p && -z ${seen[$p]:-} ]]&&{ printf '%s\n' "$base/$p";seen[$p]=1;};done < <(awk -F= '/^Default=/ {print $2}' "$base/installs.ini");fi
  if [[ -f $base/profiles.ini ]];then while IFS= read -r p;do [[ -d $base/$p && -z ${seen[$p]:-} ]]&&{ printf '%s\n' "$base/$p";seen[$p]=1;};done < <(awk -F= '/^Path=/ {print $2}' "$base/profiles.ini");fi
}
...
_expand_wildcards(){
  ...
  if [[ $path == *\** ]];then
    shopt -s nullglob;local -a items=("$path")
    for item in "${items[@]}";do [[ -e $item ]] && result_ref+=("$item");done
    shopt -u nullglob
  else [[ -e $path ]] && result_ref+=("$path");fi
}
clean_paths(){ local paths=("$@") path existing_paths=();for path in "${paths[@]}";do _expand_wildcards "$path" existing_paths;done;[[ ${#existing_paths[@]} -gt 0 ]] && rm -rf --preserve-root -- "${existing_paths[@]}" &>/dev/null||:;}
clean_with_sudo(){ local paths=("$@") path existing_paths=();for path in "${paths[@]}";do _expand_wildcards "$path" existing_paths;done;[[ ${#existing_paths[@]} -gt 0 ]] && sudo rm -rf --preserve-root -- "${existing_paths[@]}" &>/dev/null||:;}
_DOWNLOAD_TOOL_CACHED=""
get_download_tool(){
  local skip_aria2=0;[[ ${1:-} == --no-aria2 ]] && skip_aria2=1
  if [[ -n $_DOWNLOAD_TOOL_CACHED && $skip_aria2 -eq 0 ]];then printf '%s' "$_DOWNLOAD_TOOL_CACHED";return 0;fi
  local tool
...
download_file(){
  local url=$1 output=$2 tool;tool=$(get_download_tool)||return 1
  case $tool in
...
update(){ sudo rm -f /var/lib/pacman/db.lck &>/dev/null||:;sudo pacman -Syu --noconfirm;has paru && paru -Syu --noconfirm --skipreview;}
mirrorfix(){ log "Fix mirrors";has cachyos-rate-mirrors && sudo cachyos-rate-mirrors;}
cache(){ sudo rm -r /var/cache/pacman/pkg/*;has paru && paru -Scc --noconfirm||sudo pacman -Scc --noconfirm;}
log "Fix SSH/GPG permissions";sudo chmod -R 700 ~/.{ssh,gnupg}
log "Fix keyrings";sudo rm -rf /etc/pacman.d/gnupg/ /var/lib/pacman/sync
sudo pacman -Sy archlinux-keyring --noconfirm;sudo pacman-key --init;sudo pacman-key --populate
sudo pacman-key --recv-keys F3B607488DB35A47 --keyserver keyserver.ubuntu.com;sudo pacman-key --lsign-key F3B607488DB35A47;sudo pacman-key --lsign cachyos;sudo pacman-key --refresh-keys
log "Fix base-devel";sudo pacman -Sy --needed base-devel --noconfirm
...
sudo mkdir -p /etc/gnupg;sudo cp ~/.local/share/omarchy/default/gpg/dirmngr.conf /etc/gnupg/;sudo chmod 644 /etc/gnupg/dirmngr.conf
sudo gpgconf --kill dirmngr||:;sudo gpgconf --launch dirmngr||:
