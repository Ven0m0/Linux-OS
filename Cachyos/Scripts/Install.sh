...
run_priv modprobe zram tcp_bbr || :
...
if (( ${#miss[@]} )); then
  ...
  if (( aur )); then
    ...
    run_priv pacman -Sq --needed --noconfirm "${miss[@]}" 2>/dev/null || die "Install failed"
  fi
  log "Packages ✓"
else
  log "All packages ✓"
...
zsh_setup(){
  ...
  # antidote (replaces zinit)
  if [[ ! -d "$HOME/.local/share/antidote" ]]; then
    git clone --depth=1 https://github.com/mattmc3/antidote.git "$HOME/.local/share/antidote" 2>/dev/null || :
  fi
}
...
if has update-initramfs; then
  run_priv update-initramfs 2>/dev/null || :
elif has limine-mkinitcpio; then
  run_priv limine-mkinitcpio 2>/dev/null || :
elif has mkinitcpio; then
  run_priv mkinitcpio -P 2>/dev/null || :
elif has /usr/lib/booster/regenerate_images; then
  run_priv /usr/lib/booster/regenerate_images 2>/dev/null || :
elif has dracut-rebuild; then
  run_priv dracut-rebuild 2>/dev/null || :
else
