...
install_packages(){
  ...
  declare -A have; for p in "${installed[@]}"; do have[$p]=1; done
  local -a missing; for p in "${pkgs[@]}"; do [[ -n ${have[$p]:-} ]] || missing+=("$p"); done
  (( ${#missing[@]} == 0 )) && { msg "All packages installed"; return 0; }
