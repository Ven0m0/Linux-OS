...
pp_normalize_redirects(){
  [[ $HAS_SD -eq 1 ]] && sd '&>/dev/null' '>/dev/null 2>&1' || sed 's|&>/dev/null|>/dev/null 2>\&1|g'
}
...
donull(){
  [[ $HAS_SD -eq 1 ]] && sd '&>/dev/null' '>/dev/null 2>&1' "$1" || sed -i 's|&>/dev/null|>/dev/null 2>\&1|g' "$1"
}
...
docheck(){ shellcheck -a -x -s bash -P SCRIPTDIR -f diff -- "$1" | patch -Nlp1 >/dev/null 2>&1 || :; }
doharden(){ shellharden --replace -- "$1" >/dev/null 2>&1 || :; }
