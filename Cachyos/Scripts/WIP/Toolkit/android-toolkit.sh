...
die(){ log err "$*"; exit 1; }
confirm(){ [[ $YES -eq 1 ]] && return 0; local p="${1:-Proceed?} [y/N] "; read -r -p "$p" ans; [[ ${ans,,} == y* ]]; }
...
ensure_conn(){
  ...
  if [[ -n $DEVICE ]]; then
    ...
    if ((${#devs[@]}>1)); then
      DEVICE="${devs[0]}"; log warn "Multiple devices; using: $DEVICE"
      ADB=(adb -s "$DEVICE")
    else
      DEVICE="${devs[0]}"; ADB=(adb)
    fi
  fi
...
dev_sh(){
  local cmd="$1"
  if [[ $DRYRUN -eq 1 ]]; then log dbg "would: $cmd"; return 0; fi
  if [[ $USE_SHIZUKU -eq 1 ]]; then rish sh -lc "$cmd"; else "${ADB[@]}" shell sh -lc "$cmd"; fi
}
# Capture output (trim trailing CR)
dev_out(){ local cmd="$1" out
  if [[ $DRYRUN -eq 1 ]]; then printf '\n'; return 0; fi
  if [[ $USE_SHIZUKU -eq 1 ]]; then out="$(rish sh -lc "$cmd" 2>/dev/null || :)"
...
device_info(){
  local model ver; model="$(dev_out 'getprop ro.product.model')" ver="$(dev_out 'getprop ro.build.version.release')"
...
clean_downloads(){
  local days="${1:-45}"
  confirm "Delete files in Download older than $days days?" || { log warn "skip downloads"; return 0; }
...
show_stats(){
  ...
  printf 'Free /data: %s\n' "$(free_space)"
  dev_sh "df -h /data" || :
  log info "Top 10 apps by /data usage:"; dev_sh "du -h /data/data | sort -hr | head -10" || :
  log info "Top 10 largest files (Download/DCIM):"; top_files || :
}
...
apply_profile(){
  ...
  case "$prof" in
    performance) opt_system; opt_graphics; opt_input; opt_art ;;
    battery) opt_battery; opt_doze; opt_privacy; put_setting global animator_duration_scale 0.5; put_setting global transition_animation_scale 0.5; put_setting global window_animation_scale 0.5 ;;
    balanced|*) opt_system; opt_graphics; opt_battery; opt_art; put_setting global animator_duration_scale 0.3; put_setting global transition_animation_scale 0.3; put_setting global window_animation_scale 0.3 ;;
  esac
}

# PERMISSIONS (from TOML [permission])
grant_perm(){ local mode="$1" pkg="$2"
  case "$mode" in
    ...
    *) log warn "unknown perm: $mode"; return 1 ;;
  esac
}
apply_permissions_from_toml(){
  [[ -f "$CONFIG_FILE" ]] || { log warn "config not found: $CONFIG_FILE"; return 1; }
...
wa_opt_images(){
  [[ "${WA_OPTIMIZE_IMAGES:-1}" -eq 1 ]] || { log warn "WA image optimization disabled"; return 0; }
...
device_config_apply(){
  ...
  opt_connectivity; opt_privacy; opt_battery; opt_graphics; opt_system
}
device_config_reset(){
...
# BACKUP/RESTORE
pkg_backup(){ local out="${1:-pkglist-$(date +%Y%m%d).txt}"; dev_out "pm list packages | cut -d: -f2" >"$out"; printf 'Saved to %s\n' "$out"; }
pkg_restore(){ local file="$1"; [[ -f $file ]] || die "restore file missing: $file"
  while IFS= read -r p; do [[ -z $p ]] && continue; dev_sh "pm install \"$p\"" || log warn "install failed: $p"; done < "$file"
}

# COMMANDS
cmd_info(){ ensure_conn; device_info; }
cmd_stats(){ ensure_conn; show_stats; }
cmd_clean(){
  local sys=0 dl_days=0 no_browser=0 no_thumbs=0
  while [[ $# -gt 0 ]]; do case "$1" in
    --system-apps) sys=1; shift ;;
    --downloads) dl_days="${2:-0}"; shift 2 ;;
    --no-browser) no_browser=1; shift ;;
    --no-thumbnails) no_thumbs=1; shift ;;
    -h|--help) usage_clean; return 0 ;;
    *) log err "unknown: $1"; usage_clean; return 1 ;;
  esac; done
  ensure_conn
  export CLEAN_SYSTEM_APPS=$sys
  local before after; before="$(free_space)"
  log info "Cleanup start. Free /data: $before"
  clean_app_caches; clean_logs; clean_temp
  [[ $no_browser -eq 0 ]] && clean_browser
  [[ $no_thumbs -eq 0 ]] && clean_thumbs
  ((dl_days>0)) && clean_downloads "$dl_days"
  after="$(free_space)"; printf 'Free /data: %s -> %s\n' "$before" "$after"
}
cmd_optimize(){
  local prof="" dns="" cats=()
  while [[ $# -gt 0 ]]; do case "$1" in
    ...
    --dns) dns="$2"; shift 2 ;;
    art|connectivity|privacy|battery|graphics|audio|input|webview|system|doze|all) cats+=("$1"); shift ;;
    -h|--help) usage_opt; return 0 ;;
    *) log err "unknown: $1"; usage_opt; return 1 ;;
  esac; done
  ensure_conn
  [[ -n $dns ]] && { put_setting global private_dns_mode hostname; put_setting global private_dns_specifier "$dns"; }
  if [[ -n $prof ]]; then apply_profile "$prof"; log info "profile applied: $prof"; return 0; fi
  ((${#cats[@]}==0)) && cats=(all)
  ...
  for c in "${cats[@]}"; do
    case "$c" in
      ...
      system) opt_system ;;
      doze) opt_doze ;;
      all) opt_art; opt_connectivity; opt_privacy; opt_battery; opt_graphics; opt_audio; opt_input; opt_webview; opt_system; opt_doze ;;
    esac
  done
}
cmd_permissions(){
  ...
  while [[ $# -gt 0 ]]; do case "$1" in
    list|add|apply|grant) action="$1"; shift ;;
    -h|--help) usage_perm; return 0 ;;
    *) if [[ -z $pkg ]]; then pkg="$1"; shift; elif [[ -z $perm ]]; then perm="$1"; shift; else log err "too many args"; usage_perm; return 1; fi ;;
  esac; done
  ensure_conn
  case "$action" in
    list) [[ -f $CONFIG_FILE ]] || die "no config: $CONFIG_FILE"; awk '/^\[permission\]/{p=1;next} /^\[/{p=0} p && !/^[[:space:]]*#/{print}' "$CONFIG_FILE" ;;
    add)
      [[ -n $pkg && -n $perm ]] || { usage_perm; return 1; }
...
cmd_whatsapp(){
  ...
  while [[ $# -gt 0 ]]; do case "$1" in
    ...
    -h|--help) usage_wa; return 0 ;;
    *) log err "unknown: $1"; usage_wa; return 1 ;;
  esac; done
  ensure_conn
  local before after; before="$(free_space)"
  wa_clean_opus "$days"; wa_clean_paths; [[ $no_images -eq 0 ]] && WA_OPTIMIZE_IMAGES=1 wa_opt_images
  after="$(free_space)"; printf 'Free /data: %s -> %s\n' "$before" "$after"
}
cmd_backup(){ ensure_conn; pkg_backup "${1:-}"; }
cmd_restore(){ ensure_conn; [[ $# -gt 0 ]] || die "restore needs a file"; pkg_restore "$1"; }
cmd_devcfg(){ ensure_conn; case "${1:-apply}" in apply) device_config_apply ;; reset) device_config_reset ;; *) usage_devcfg; return 1 ;; esac; }
...
main(){
  ...
  case "$cmd" in
    ...
    help|-h|--help) usage ;;
    *) log err "unknown command: $cmd"; usage; exit 1 ;;
  esac
}
main "$@"
