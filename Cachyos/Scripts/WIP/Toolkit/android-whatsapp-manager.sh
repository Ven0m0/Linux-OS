...
check_requirements() {
  if [[ $USE_SHIZUKU -eq 1 ]]; then
    has rish || {
      log error "rish command not found. Install Shizuku and set up rish."
      exit 1
    }
    # Test rish connection
    rish id >/dev/null 2>&1 || {
      log error "rish connection failed. Is Shizuku running?"
      exit 1
    }
  else
    has adb || {
      log error "adb command not found. Install Android SDK Platform Tools."
      exit 1
    }
    # Test adb connection
