...
class Counters:
  ...
  def __add__(self, other):
    ...
    return Counters(
      ...
      decrypted_cnt=self.decrypted_cnt + other.decrypted_cnt,
      converted_cnt=self.converted_cnt + other.converted_cnt,
...
def process_conversion_task(
  root: Path, cia_file: Path, tools_list: list[Path], seeddb_path: Path
) -> Counters:
  """
...
def main() -> None:
  ...
  print(f"  - {cnt.count_cia} CIA file(s) found")
  print(f"  - {cnt.decrypted_cnt} file(s) decrypted")
  if cnt.convert_to_cci:
