...
def sanitize_filename(name: str) -> str:
    ...
    out = "".join(c if c.lower() in VALID_FILENAME_CHARS else "" for c in name)
...
def parse_twl_ctrtool_output(text: str) -> TitleInfo:
    ...
    info = TitleInfo()
    for line in text.splitlines():
        if m := TWL_TITLE_ID_RE.search(line):
            info.title_id = m.group(1)
        if m := TITLE_VERSION_RE.search(line):
            info.title_version = m.group(1)
        if m := TWL_ENCRYPTED_RE.search(line):
...
CIA_TYPE_PATTERNS = {
    ...
    "System": re.compile(r"00040010|0004001b|00040030|0004009b|000400db|00040130|00040138"),
    "Demo": re.compile(r"00040002"),
    "Patch": re.compile(r"0004000e"),
    "DLC": re.compile(r"0004008c"),
}
...
def decrypt_cia(
    ...
    if "Secure" not in info.crypto_key:
        ...
        if twl_info.crypto_key.upper() == "YES" and tid.startswith("00048"):
            ...
            run_tool(
                ctrtool,
                ["--contents=bin/00000000.app", "--meta=bin/00000000.app", str(file)],
                cwd=root,
            )
...
UNSUPPORTED_CCI_PATTERN = re.compile(
...
def convert_cia_to_cci(
    ...
    if tid_match:
        ...
        if UNSUPPORTED_CCI_PATTERN.search(tid):
            logging.error(
