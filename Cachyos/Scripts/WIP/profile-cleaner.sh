...
for cmd in bc find parallel sqlite3 xargs file; do
  command -v "$cmd" >/dev/null || { echo >&2 "Missing: $cmd"; exit 1; }
done
export GRN YLW NRM
do_clean_file() {
...
scan_and_clean() {
  ...
  shift 3; local subdirs=("$@")
  printf " ${YLW}Checking %s...${NRM}\n" "$name"
  if [[ "$type" == "chrome" ]]; then
    for dir in "${subdirs[@]}"; do [[ -d "$base/$dir" ]] && paths_to_clean+=("$base/$dir"); done
    [[ ${#paths_to_clean[@]} -eq 0 ]] && { echo -e "${RED}Error: No profiles found for $name${NRM}"; exit 1; }
  elif [[ "$type" == "mozilla" ]]; then
    [[ ! -d "$base" || ! -f "$base/profiles.ini" ]] && { echo -e "${RED}Error: Invalid path or profiles.ini missing${NRM}"; exit 1; }
    while read -r line; do
      local p="${line#*=}"; [[ -d "$base/$p" ]] && paths_to_clean+=("$base/$p") || paths_to_clean+=("$p")
    ...
    [[ ${#paths_to_clean[@]} -eq 0 ]] && { echo -e "${RED}Error: Invalid path(s)${NRM}"; exit 1; }
  fi
...
case "$1" in
  ...
  n|N) nb="$HOME/.newsboat"; [[ -d "$XDG_DATA_HOME/newsboat" ]] && nb="$XDG_DATA_HOME/newsboat"; scan_and_clean simple "Newsboat" "$nb" ;;
  TO|to) base="$HOME/.torbrowser/profile"; for l in de en es fr it ru; do [[ ! -d "$base" ]] && base="$HOME/.tor-browser-$l/INSTALL/Data/profile"; done; scan_and_clean simple "TorBrowser" "$base" ;;
  P|p) shift; scan_and_clean path "Custom Paths" "$@" ;;
