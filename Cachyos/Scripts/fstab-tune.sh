...
download_file() {
  ...
  case $tool in aria2c) aria2c -q --max-tries=3 --retry-wait=1 -d "$(dirname "$output")" -o "$(basename "$output")" "$url" ;; curl) curl -fsSL --retry 3 --retry-delay 1 "$url" -o "$output" ;; wget2) wget2 -q -O "$output" "$url" ;; wget) wget -qO "$output" "$url" ;; *) return 1 ;; esac
}
...
select profile in "Desktop (balanced)" "Server (performance)" "Custom"; do
  ...
  "Server (performance)")
    ...
    ;;
  "Custom")
    read -rp "Enter custom mount options (comma-separated): " opts
    break
    ;;
  *) echo "Invalid choice." ;;
  esac
