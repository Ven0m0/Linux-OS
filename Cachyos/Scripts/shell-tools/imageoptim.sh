...
compress_image(){
  ...
  ext="${file##*.}"; ext="${ext,,}"
  backup_dir="$BACKUP_DIR/$(dirname -- "$file")"
  mkdir -p "$backup_dir"
  if [ "$DRYRUN" -eq 1 ]; then
  ...
  case "$ext" in
        ...
        if [ "$DRYRUN" -eq 1 ]; then
          ...
          if pngquant --strip --quality=60-85 --speed=1 --output "$tmpfile" -- "$file"; then
            if command -v oxipng >/dev/null 2>&1; then
              oxipng -o max --strip all -a -i 0 --scale16 --force -Z --zi 25 -j "$NPROC" --out "$file" "$tmpfile" || true
            else
              mv -f -- "$tmpfile" "$file"
            fi
          else
            rm -f -- "$tmpfile" || true
            if command -v oxipng >/dev/null 2>&1; then
              if [ "$DRYRUN" -eq 1 ]; then printf '[DRY] oxipng (fallback) %s\n' "$file" >>"$LOGFILE"; else oxipng -o max --strip all -a -i 0 --force -Z --zi 20 -- "$file" || true; fi
            else
    ...
    gif)
      ...
      if command -v gifsicle >/dev/null 2>&1; then
        ...
        if [ "$DRYRUN" -eq 1 ]; then
          ...
          ffmpeg -i "$file" -hide_banner -loglevel error "$tmpdir/frame%06d.png" || { printf '[WARN] ffmpeg failed for %s\n' "$file" >>"$LOGFILE"; rm -rf -- "$tmpdir"; }
    ...
    svg)
      if command -v svgo >/dev/null 2>&1; then
        if [ "$DRYRUN" -eq 1 ]; then printf '[DRY] svgo %s\n' "$file" >>"$LOGFILE"; else svgo --multipass --quiet -- "$file" || true; fi
      elif command -v scour >/dev/null 2>&1; then
        if [ "$DRYRUN" -eq 1 ]; then printf '[DRY] scour %s\n' "$file" >>"$LOGFILE"; else scour -i "$file" -o "$file.tmp" --enable-id-stripping --enable-comment-stripping || true; mv -f -- "$file.tmp" "$file" 2>/dev/null || true; fi
      else
...
"${FD_CMD[@]}" | {
  if command -v rust-parallel >/dev/null 2>&1; then
    ...
    done | rust-parallel -s --jobs "$JOBS" --null-separator || true
    exit
  fi

  if command -v xargs >/dev/null 2>&1; then
    # xargs handles NULs safely; call compress_image in a bash -c so exported function is used
    xargs -0 -P "$JOBS" -I {} bash -lc 'compress_image "$@"' _ {} || true
    exit
  fi

  if command -v parallel >/dev/null 2>&1; then
    parallel -0 -j "$JOBS" --no-notice bash -c 'compress_image "$@"' _ {} || true
    exit
  fi
