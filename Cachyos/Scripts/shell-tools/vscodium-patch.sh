...
xdg_patch(){
  ...
  [[ ${#files[@]} -eq 0 ]] && { warn "No XDG files found"; return 0; }
  local f mime_added
  for f in "${files[@]}"; do
    case $f in
      *.desktop)
        mime_added=0
        grep -q "text/plain" "$f" || { sed -i 's/^\(MimeType=.*\);$/\1;text/plain;/' "$f"; ((mime_added++)); }
        grep -q "inode/directory" "$f" || { sed -i 's/^\(MimeType=.*\);$/\1;inode\/directory;/' "$f"; ((mime_added++)); }
        ((mime_added)) && ok "$f (+${mime_added} MIME)" || ok "$f (current)" ;;
      */package.json) sed -i 's/"desktopName":\s*"\(.*\)-url-handler\.desktop"/"desktopName": "\1.desktop"/' "$f" && ok "$f" ;;
    esac
  done
}
...
json_op(){
  ...
  case $op in
    apply)
      ...
      "$JQ" -r .p "$tmp" > "$prod" && "$JQ" -r .c "$tmp" > "$cache" && rm "$tmp" && ok "Applied → $prod" ;;
    restore)
...
vscodium_prod_full(){
  ...
  local v work="/tmp/vp.$$" src="${work}/product.json" bak="${dst}.backup.$(date +%s)"
...
vscodium_restore(){
  local d=${1:-/usr/share/vscodium/resources/app/product.json} b
  mapfile -t b < <(find "${d%/*}" -maxdepth 1 -name "${d##*/}.backup.*" -printf "%T@ %p\n" 2>/dev/null | sort -rn | head -1)
  [[ ${#b[@]} -eq 0 ]] && die "No backup found for $d"
  b=${b[0]#* }
