...
settings_files=[f'{home_dir}/.config/Code/User/settings.json',f'{home_dir}/.var/app/com.visualstudio.code/config/Code/User/settings.json',f'{home_dir}/.config/VSCodium/User/settings.json',f'{home_dir}/.var/app/com.vscodium.codium/config/VSCodium/User/settings.json']
changed=0
for sf in settings_files:
  ...
  except json.JSONDecodeError:print(f'Invalid JSON in {sf}',file=sys.stderr);continue
...
xdg_patch(){
  ...
  for f in "${files[@]}";do
    case $f in
      *.desktop) mime_added=0
        grep -qF "text/plain" "$f"||{ sed -i 's/^\(MimeType=.*\);$/\1;text\/plain;/' "$f";((++mime_added));}
        grep -qF "inode/directory" "$f"||{ sed -i 's/^\(MimeType=.*\);$/\1;inode\/directory;/' "$f";((++mime_added));}
        ((mime_added)) && ok "$f (+${mime_added} MIME)"||ok "$f (current)";;
      */package.json) sed -i 's/"desktopName":[[:space:]]*"\([^"]*\)-url-handler\.desktop"/"desktopName": "\1.desktop"/' "$f" && ok "$f";;
    esac
  done
}
json_op(){
...
repo_swap(){
  ...
  if ((mode));then
    ...
    sed -i -e 's|"serviceUrl":.*|"serviceUrl": "https://marketplace.visualstudio.com/_apis/public/gallery",|' -e '/"cacheUrl"/d' -e '/"serviceUrl"/a\    "cacheUrl": "https://vscode.blob.core.windows.net/gallery/index",' -e 's|"itemUrl":.*|"itemUrl": "https://marketplace.visualstudio.com/items"|' -e '/"linkProtectionTrustedDomains"/d' "$f"
...
vscodium_prod_full(){
  ...
  rm -rf "$work";ok "VSCodium Full Patch (backup: $bak)"
}
vscodium_restore(){
...
configure_privacy(){
  ...
  for setting in "${settings[@]}";do
    ...
    vscode_json_set "$prop" "$val" && { printf '  %b %s\n' "${G}✓${D}" "$prop";((changed++));}||printf '  %b %s\n' "${Y}→${D}" "$prop"
  done
...
main(){
  ...
  case ${1:-} in
    xdg) xdg_patch;;
    xdg-data) local f=${2:-/usr/share/vscodium/resources/app/product.json};sed -i 's|"dataFolderName": "[^"]*"|"dataFolderName": ".local/share/codium"|' "$f" && ok "DataFolder";;
    ...
    mkt) json_op apply "${2:-$CP}" "${3:-$CD/code-marketplace/patch.json}" "${4:-$CD/code-marketplace/cache.json}";sign_fix node-ovsx-sign;;
    mkt-restore) json_op restore "${2:-$CP}" "${3:-$CD/code-marketplace/patch.json}" "${4:-$CD/code-marketplace/cache.json}";sign_fix;;
    mkt-update) local -a K=(extensionsGallery extensionRecommendations keymapExtensionTips languageExtensionTips configBasedExtensionTips webExtensionTips virtualWorkspaceExtensionTips remoteExtensionTips extensionAllowedBadgeProviders extensionAllowedBadgeProvidersRegex msftInternalDomains linkProtectionTrustedDomains);update_json "${2:-}" "${3:-./patch.json}" K;;
    privacy) configure_privacy;;
    all) xdg_patch;repo_swap "" 0;json_op apply "$CP" "$CD/code-marketplace/patch.json" "$CD/code-marketplace/cache.json";json_op apply "$CP" "$CD/code-features/patch.json" "$CD/code-features/cache.json";sign_fix node-ovsx-sign;configure_privacy;;
