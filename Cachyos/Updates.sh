...
main(){
  ...
  update_extras(){
    ...
    if has mise;then mise p i -ay;mise prune -y;mise up -y||:;fi
    if has bun;then bun update -g --latest||bun update -g
    elif has pnpm;then pnpm up -Lg||:
  ...
  update_maintenance(){
    ...
    if has mkinitcpio;then sudo mkinitcpio -P||:
    elif has dracut;then sudo dracut --regenerate-all --force||:
  ...
  update_system;update_extras;update_python;update_maintenance
  log "\n${GRN}All done ✅${DEF}"
}
main "$@"
