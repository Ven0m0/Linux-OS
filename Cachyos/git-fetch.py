...
def fetch_github(spec: RepoSpec, output: Path) -> None:
  ...
  api_url = f"https://api.github.com/repos/{spec.owner}/{spec.repo}/contents/{spec.path}"
  if spec.branch != 'main':
...
def fetch_gitlab(spec: RepoSpec, output: Path) -> None:
  ...
  params = {'path': spec.path, 'ref': spec.branch, 'per_page': '100', 'recursive': 'true'}
...
def main() -> int:
  if len(sys.argv) < 2:
    ...
    print("  fetch.py https://github.com/owner/repo/tree/main/path", file=sys.stderr)
    print("  fetch.py https://gitlab.com/owner/repo/-/tree/main/path output/", file=sys.stderr)
