...
set -euo pipefail; shopt -s nullglob globstar; sync
export LC_ALL=C LANG=C.UTF-8 HOME="/home/${SUDO_USER:-$USER}"
...
command -v rate-mirrors &>/dev/null || die "install rate-mirrors"
...
rate_repository_mirrors(){
  local repo="$1" path="$2"
  info "Ranking '%s'..." "$repo"
  if rate-mirrors --save="$TMP_MAIN" --allow-root --per-mirror-timeout 300000 "$repo"; then
    if [ ! -f "$path" ]; then
      die "target '%s' missing, cannot write" "$path"
    fi
...
rank_arch_from_url(){
  ...
  mapfile -t urls < <(awk '/^[[:space:]]*#?Server[[:space:]]*=/{ url=$3; sub(/\$.*/,"",url); if(substr(url,length(url),1)!="/") url=url"/"; if(!seen[url]++) print url }' "$TMP_DL")
...
main(){
  ...
  while IFS= read -r -d '' f; do
    # skip symlinks, main file and backups
    [ -L "$f" ] && continue
    [ "$f" = "$MIRRORS_DIR/mirrorlist" ] && continue
    case "$f" in
      *.bak|*.bak.*) continue ;;
    esac
    repo=${f##*/}; repo=${repo%.bak}; repo=${repo%-mirrorlist}; repo=${repo%%-v[0-9]*}
    [ -z "$repo" ] && continue
    rate_repository_mirrors "$repo" "$f" &
  done < <(find "$MIRRORS_DIR" -maxdepth 1 -type f -name "*mirrorlist*" ! -name "*.bak" ! -name "*.bak.*" -print0)
