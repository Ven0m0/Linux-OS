...
set -Eeuo pipefail; shopt -s nullglob globstar; IFS=$'\n\t'
...
trap cleanup EXIT; trap 'err "failed at line $LINENO"' ERR
usage(){
  cat << 'EOF'
...
parse_args(){
  while (($#)); do
    case "$1" in
      ...
      -q | --quiet) cfg[quiet]=1; exec >/dev/null ;;
      -h | --help) usage; exit 0 ;;
      --) shift; break ;;
...
EOF
  ...
  root_dev=$(findmnt -n -o SOURCE /); home_dev=$(findmnt -n -o SOURCE /home 2>/dev/null || echo "$root_dev")
  [[ -n $root_dev ]] && {
...
install_core_tools(){
  ...
  [[ -f /usr/bin/fdfind && ! -f "$HOME/.local/bin/fd" ]] && {
    mkdir -p "$HOME/.local/bin"; ln -sf /usr/bin/fdfind "$HOME/.local/bin/fd"
  }
}
install_extended_tools(){
  ((cfg[minimal])) && return 0; log "Installing extended tooling suite"
  if ! has eza; then
...
install_package_managers(){
  ((cfg[minimal] || cfg[skip_external])) && return 0; log "Installing alternative package managers"
  ...
  if ! has eget; then
    ...
    [[ -f ./eget ]] && { mkdir -p "$HOME/.local/bin"; mv ./eget "$HOME/.local/bin/"; }
  fi
  ! has pacstall && sudo bash -c "$(curl -fsSL https://pacstall.dev/q/install)"
}
# External Installers (Pi-hole, PiKISS, PiApps)
install_external(){
  ((cfg[skip_external])) && return 0; log "Running external installers (interactive)"
  if ! has pihole; then
...
# Main Execution
main(){
  parse_args "$@"; log "${BLD}Raspberry Pi Setup & Optimization${DEF}"
  log "Mode: $([[ ${cfg[dry_run]} -eq 1 ]] && echo 'DRY-RUN' || echo 'LIVE')"
  log "Profile: $([[ ${cfg[minimal]} -eq 1 ]] && echo 'MINIMAL' || echo 'FULL')"
  configure_apt; enable_ip_forwarding; configure_dpkg_nodoc; clean_docs
  optimize_system; configure_ssh; install_core_tools; install_extended_tools
  install_package_managers; install_external
  sudo apt-get autoremove -y; sudo apt-get autoclean
  has flatpak && run flatpak uninstall --unused --delete-data -y || :
  log "${GRN}✓${DEF} Setup complete"; warn "Reboot recommended to apply all optimizations"
}
main "$@"
