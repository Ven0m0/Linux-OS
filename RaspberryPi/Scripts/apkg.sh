...
find_with_fallback(){
  local ftype="${1:--f}" pattern="${2:-*}" search_path="${3:-.}" action="${4:-}"; shift 4 2>/dev/null || shift $#
  if has fdf; then fdf -H -t "$ftype" "$pattern" "$search_path" ${action:+"$action"} "$@"
  elif has fd; then fd -H -t "$ftype" "$pattern" "$search_path" ${action:+"$action"} "$@"
  else
    local find_type_arg
...
CACHE_DIR="${XDG_CACHE_HOME%/}/apkg"; CACHE_INDEX="${CACHE_DIR}/.index"
...
has fzf || exit 1
find_cache_files(){ :; }
if has fd; then FIND_TOOL="fd"; find_cache_files(){ fd -0 -d 1 -tf . "$CACHE_DIR" 2>/dev/null; }
elif has fdfind; then FIND_TOOL="fdfind"; find_cache_files(){ fdfind -0 -d 1 -tf . "$CACHE_DIR" 2>/dev/null; }
else FIND_TOOL="find"; find_cache_files(){ find "$CACHE_DIR" -maxdepth 1 -type f -print0 2>/dev/null; }
fi
FINDER_OPTS=(--layout=reverse-list --no-hscroll)
[[ -n ${APT_FUZZ_FINDER_OPTS:-} ]] && read -r -a FINDER_OPTS <<< "$APT_FUZZ_FINDER_OPTS"
# Manager detection
declare -A MANAGERS=([apt]=1); has nala && MANAGERS[nala]=1; has apt-fast && MANAGERS[apt-fast]=1
...
_update_cache_index(){
  ...
  find_cache_files | xargs -0 -r stat -c '%n|%s|%Y' >"$tmp" 2>/dev/null || :; mv -f "$tmp" "$CACHE_INDEX"
}
_cache_info_from_index(){ awk -F'|' 'BEGIN{t=f=o=0}{f++;t+=$2;if(!o||$3<o)o=$3}END{print t,f,o}' "$CACHE_INDEX" 2>/dev/null || echo "0 0 0"; }
evict_old_cache(){
  [[ ! -s $CACHE_INDEX ]] && _update_cache_index; [[ ! -s $CACHE_INDEX ]] && return
  local now=$(date +%s) limit="$APT_FUZZ_CACHE_MAX_BYTES" del cutoff valid
  cutoff=$((now - APT_FUZZ_CACHE_TTL)); del=$(mktemp); valid=$(mktemp); trap 'rm -f "$del" "$valid"' RETURN
  ...
  xargs -r -a "$del" rm -f --; _update_cache_index
}
# Cache & preview
_cache_file_for(){ printf '%s/%s.cache' "$CACHE_DIR" "${1//[^a-zA-Z0-9._+-]/_}"; }
_generate_preview(){
  local pkg="$1" out tmp; out="$(_cache_file_for "$pkg")"; tmp="${out}.tmp.$$"
  {
    apt-cache show "$pkg" 2>/dev/null || :
    printf '\n--- changelog (first 200 lines) ---\n'
    apt-get changelog "$pkg" 2>/dev/null | sed -n '1,200p' || :
  } >"$tmp" 2>/dev/null
  sed -i 's/\x1b\[[0-9;]*m//g' "$tmp" 2>/dev/null || :; mv -f "$tmp" "$out"; chmod 644 "$out" 2>/dev/null || :
}
_cached_preview_print(){
  local pkg="$1" f now mtime; f="$(_cache_file_for "$pkg")"; now=$(date +%s); mtime=$(stat -c %Y -- "$f" 2>/dev/null || echo 0)
  if ((now - mtime < APT_FUZZ_CACHE_TTL)); then cat "$f" 2>/dev/null || echo "(no preview)"
  else _generate_preview "$pkg"; cat "$f" 2>/dev/null || echo "(no preview)"; fi
}
...
_prefetch_previews(){
  ...
  while IFS= read -r pkg; do
    ((i = i % APT_FUZZ_PREFETCH_JOBS, i++ == 0)) && wait; _generate_preview "$pkg" &
...
# Manager runner
run_mgr(){
  local action="$1"; shift; local -a pkgs=("$@") cmd=()
...
_status_header(){
  ...
  ((oldest == 0)) && age="0m" || age=$(((now - oldest) / 60))m; size=$(byte_to_human "$total")
  printf 'manager: %s | cache: %s files, %s, oldest: %s\n' "$PRIMARY_MANAGER" "$files" "$size" "$age"
}
action_menu_for_pkgs(){
  local -a pkgs=("$@") actions=(Install Remove Purge Changelog Cancel) choice; [[ ${#pkgs[@]} -eq 0 ]] && return
...
menu_search(){
  ...
  [[ -z $sel ]] && return; local -a pkgs; mapfile -t pkgs <<< "$sel"; action_menu_for_pkgs "${pkgs[@]}"
}
menu_installed(){
...
menu_upgradable(){
  local sel=$(list_upgradable | fzf "${FINDER_OPTS[@]}" --multi --height=40% --prompt="Upgradable> " --header="$(_status_header)" \
    --preview="bash -c '_cached_preview_print {}'" --preview-window=right:60% --bind 'tab:toggle+down,ctrl-a:select-all,ctrl-d:deselect-all')
  [[ -z $sel ]] && return; local -a pkgs; mapfile -t pkgs <<< "$sel"
  [[ ${#pkgs[@]} -gt 0 ]] && run_mgr install "${pkgs[@]}"
}
backup_installed(){
  local out="${1:-pkglist-$(date +%F).txt}"; dpkg-query -W -f='${Package}\n' | sort -u >"$out"; printf 'Saved: %s\n' "$out"
}
restore_from_file(){
  local file="$1"; [[ ! -f $file ]] && { echo "File not found: $file" >&2; return 1; }
  local -a pkgs; mapfile -t pkgs < <(sed '/^$/d' "$file"); [[ ${#pkgs[@]} -eq 0 ]] && { echo "No packages"; return 0; }
  local -a sel; mapfile -t sel < <(printf '%s\n' "${pkgs[@]}" | fzf "${FINDER_OPTS[@]}" --multi --height=40% --prompt="Confirm install> ")
  [[ ${#sel[@]} -eq 0 ]] && return; run_mgr install "${sel[@]}"
}
menu_backup_restore(){
  local -a opts=("Backup installed packages" "Restore from file" "Cancel") choice; local header=$(_status_header)
...
menu_maintenance(){
  local -a opts=(Update Upgrade Autoremove Clean "Choose manager" Cancel) choice; local header=$(_status_header)
...
main_menu(){
  evict_old_cache; local -a menu=("Search packages" "Installed" "Upgradable" "Backup/Restore" "Maintenance" "Choose manager" "Quit") choice header
  while true; do
...
_install_self(){
  ...
  mkdir -p -- "${HOME}/.local/bin"; cp -f -- "$0" "$dest"; chmod +x -- "$dest"; printf 'Installed: %s\n' "$dest"
  mkdir -p -- "$compdir"
...
_uninstall_self(){
  ...
  printf 'Uninstalling...\n'; rm -f -- "$dest" "$comp"; rm -rf -- "$CACHE_DIR"
  printf 'Removed: %s\n%s\n%s\n' "$dest" "$comp" "$CACHE_DIR"
}
# Entry point
if (($# > 0)); then
  case "$1" in
    ...
    install|remove|purge)
      [[ $# -lt 2 ]] && { echo "Usage: $0 $1 <pkgs...>" >&2; exit 2; }; action="$1"; shift; run_mgr "$action" "$@"; exit 0;;
...
EOF
evict_old_cache; _prefetch_lists; _prefetch_previews & main_menu
