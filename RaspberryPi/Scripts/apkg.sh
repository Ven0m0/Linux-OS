...
else echo "Install skim (sk) or fzf" >&2; exit 1; fi
...
_update_cache_index(){
  local tmp; tmp=$(mktemp "${CACHE_INDEX}.XXXXXX")
...
evict_old_cache(){
  ...
  local del; del=$(mktemp); local valid; valid=$(mktemp)
  ...
  if (( limit>0 )) && [[ -s $valid ]]; then
    local total; total=$(awk -F'|' '{s+=$2}END{print s+0}' "$valid")
    if (( total>limit )); then
...
_generate_preview(){
  local pkg="$1" out; out="$(_cache_file_for "$pkg")"
  local tmp="${out}.tmp.$$"
...
_cached_preview_print(){
  local pkg="$1" f; f="$(_cache_file_for "$pkg")"
...
# UI helpers
_status_header(){
  local total files oldest; read -r total files oldest < <(_cache_info_from_index)
  local age; (( oldest==0 )) && age="0m" || age=$((($(date +%s)-oldest)/60))m
  local size; size=$(byte_to_human "$total")
...
menu_search(){
  local sel; sel=$(list_all_packages | "$FINDER" "${FINDER_OPTS[@]}" \
  ...
  [[ -z $sel ]] && return
  local -a pkgs; mapfile -t pkgs <<< "$sel"
  action_menu_for_pkgs "${pkgs[@]}"
}

menu_installed(){
  local sel; sel=$(list_installed | "$FINDER" "${FINDER_OPTS[@]}" \
...
menu_upgradable(){
  local sel; sel=$(list_upgradable | "$FINDER" "${FINDER_OPTS[@]}" \
...
restore_from_file(){
  ...
  local -a pkgs; mapfile -t pkgs < <(sed '/^$/d' "$file")
...
if (( $# > 0 )); then
  case "$1" in
    ...
    install|remove|purge) [[ $# -lt 2 ]] && { echo "Usage: $0 $1 <pkgs...>" >&2; exit 2; }; cmd="$1"; shift; run_mgr "$cmd" "$@"; exit 0 ;;
