...
display_banner() { local banner_text="$1"; shift; local -a flag_colors=("$@"); if (( ${#flag_colors[@]} == 0 )); then flag_colors=("$LBLU" "$PNK" "$BWHT" "$PNK" "$LBLU"); fi; mapfile -t banner_lines <<<"$banner_text"; local lines=${#banner_lines[@]}; local segments=${#flag_colors[@]}; if ((lines <= 1)); then for line in "${banner_lines[@]}"; do printf "%s%s%s
" "${flag_colors[0]}" "$line" "$DEF"; done; else for i in "${!banner_lines[@]}"; do local segment_index=$((i * (segments - 1) / (lines - 1))); ((segment_index >= segments)) && segment_index=$((segments - 1)); printf "%s%s%s
" "${flag_colors[segment_index]}" "${banner_lines[i]}" "$DEF"; done; fi; }
