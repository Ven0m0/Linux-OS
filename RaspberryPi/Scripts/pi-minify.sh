...
# Disk usage tracking
track_disk(){
  local label="$1" usage; usage=$(df -h / | awk 'NR==2 {print $3 "/" $2 " (" $5 ")"}')
...
ask(){
  ...
  local prompt="$1" default="${2:-n}" reply; read -rp "${LBLU}?${DEF} ${prompt} [${default}] " reply
...
purge_packages(){
  ...
  has localepurge || sudo apt-get install -y localepurge; run localepurge
  local doc_pkgs; mapfile -t doc_pkgs < <(dpkg --list | awk '/-doc$/ {print $2}')
  ((${#doc_pkgs[@]} > 0)) && sudo apt-get purge -y "${doc_pkgs[@]}" || :
  sudo apt-get purge -y '*texlive*' 2>/dev/null || :
  local current_kernel; current_kernel=$(uname -r); local old_kernels
  mapfile -t old_kernels < <(dpkg --list | awk -v ck="$current_kernel" '$2 ~ /^linux-image-.*-generic$/ && $2 != ck {print $2}')
  ((${#old_kernels[@]} > 0)) && { log "Purging old kernels (keeping ${current_kernel})"; sudo apt-get purge -y "${old_kernels[@]}"; }
  local orphaned; mapfile -t orphaned < <(dpkg -l | awk '/^rc/ {print $2}')
  ((${#orphaned[@]} > 0)) && sudo apt-get purge -y "${orphaned[@]}" || :
}
purge_aggressive(){
  ((cfg[aggressive] == 0)) && return 0; warn "Aggressive mode: removing X11, dev packages, extras"
...
cleanup_apt(){
  log "APT cleanup: cache, orphans, deborphan"; has deborphan || sudo apt-get install -y deborphan
  sudo apt-get autoremove --purge -y; sudo apt-get autoclean -y; sudo apt-get clean -y
  local orphans; while mapfile -t orphans < <(deborphan) && ((${#orphans[@]} > 0)); do sudo apt-get purge -y "${orphans[@]}"; done
}
debloat(){
...
clean_caches(){
  ...
  run rm -rf ~/.local/share/Trash/* 2>/dev/null || :; sudo rm -rf /root/.local/share/Trash/* 2>/dev/null || :
  run rm -rf ~/snap/*/*/.local/share/Trash/* 2>/dev/null || :; run rm -rf ~/.var/app/*/data/Trash/* 2>/dev/null || :
  unset HISTFILE; run rm -f ~/.{bash,python}_history 2>/dev/null || :; sudo rm -f /root/.{bash,python}_history 2>/dev/null || :
...
clean_crash_dumps(){
  ...
  sudo rm -rf /var/crash/* 2>/dev/null || :; sudo rm -rf /var/lib/systemd/coredump/* 2>/dev/null || :
}
clean_system_logs(){
  log "Clearing system logs (journald)"; sudo journalctl --vacuum-time=1s
  sudo rm -rf /run/log/journal/* 2>/dev/null || :; sudo rm -rf /var/log/journal/* 2>/dev/null || :
}
disable_python_history(){
  log "Disabling Python history permanently"; local history_file="$HOME/.python_history"
  [[ ! -f $history_file ]] && touch "$history_file"; sudo chattr +i "$(realpath "$history_file")" 2>/dev/null || :
}
remove_popcon(){
  log "Removing Popularity Contest (popcon)"; local config_file='/etc/popularity-contest.conf'
...
remove_reportbug(){
  log "Removing reportbug packages"; has apt-get || return 0
  local pkgs=('reportbug' 'python3-reportbug' 'reportbug-gtk'); local pkg
  for pkg in "${pkgs[@]}"; do
    if status="$(dpkg-query -W --showformat='${db:Status-Status}' "$pkg" 2>&1)" && [[ $status == installed ]]; then sudo apt-get purge -y "$pkg"; fi
  done
}
clean_privacy_data(){
  ...
  sudo rm -rf {/root,/home/*}/.local/share/zeitgeist 2>/dev/null || :; run rm -rf ~/Pictures/Screenshots/* 2>/dev/null || :
  [[ -d ~/Pictures ]] && {
  ...
  run rm -f /.recently-used.xbel 2>/dev/null || :; run rm -f ~/.local/share/recently-used.xbel* 2>/dev/null || :
  run rm -f ~/snap/*/*/.local/share/recently-used.xbel 2>/dev/null || :; run rm -f ~/.var/app/*/data/recently-used.xbel 2>/dev/null || :
  run rm -rf "$HOME/.config/privacy.sexy/runs"/* 2>/dev/null || :; run rm -rf "$HOME/.config/privacy.sexy/logs"/* 2>/dev/null || :
}
# ZRAM Setup
disable_swap(){
  log "Disabling SWAP partition"
  has dphys-swapfile && { sudo dphys-swapfile swapoff; sudo dphys-swapfile uninstall; sudo update-rc.d dphys-swapfile remove; }
  sudo swapoff -a
}
enable_zram(){
...
ZRAMSCRIPT
  sudo chmod +x /usr/local/bin/zram-init; sudo /usr/local/bin/zram-init
...
EOF
  sudo systemctl daemon-reload; sudo systemctl enable zram-init.service
}
# System Tweaks
optimize_fstab(){ log "Optimizing fstab: noatime, nodiratime"; sudo sed -i 's/\(defaults\)/\1,noatime,nodiratime/' /etc/fstab; }
optimize_systemd(){ log "Reducing systemd stop timeout: 90s→5s"; sudo sed -i 's/#DefaultTimeoutStopSec=90s/DefaultTimeoutStopSec=5s/' /etc/systemd/system.conf; }
disable_extra_ttys(){
  [[ ! -f /etc/inittab ]] && return 0
  log "Disabling extra TTYs (2-6) for RAM savings"; sudo sed -i '/[2-6]:23:respawn:\/sbin\/getty 38400 tty[2-6]/s/^/#/' /etc/inittab
}
# Main Orchestration
main(){
  parse_args "$@"; log "${BLD}Raspberry Pi System Minification & Privacy Hardening${DEF}"
  log "Mode: $([[ ${cfg[dry_run]} -eq 1 ]] && echo 'DRY-RUN' || echo 'LIVE')"
  log "Interactive: $([[ ${cfg[interactive]} -eq 1 ]] && echo 'YES' || echo 'NO')"
  log "Aggressive: $([[ ${cfg[aggressive]} -eq 1 ]] && echo 'YES' || echo 'NO')"; track_disk "Before"
  configure_dpkg_nodoc; purge_docs; purge_packages; cleanup_apt; clean_caches; debloat
  clean_crash_dumps; clean_system_logs; disable_python_history; remove_popcon; remove_reportbug; clean_privacy_data
  [[ ${cfg[aggressive]} -eq 1 ]] && purge_aggressive
  ask "Disable SWAP and enable ZRAM?" y && { disable_swap; enable_zram; }
  ...
  track_disk "After"; log "${GRN}✓${DEF} Minification & privacy hardening complete"
  log "Disk before: ${cfg[disk_before]}"; log "Disk after:  ${cfg[disk_after]}"; warn "Reboot recommended to fully apply changes"
}
main "$@"
