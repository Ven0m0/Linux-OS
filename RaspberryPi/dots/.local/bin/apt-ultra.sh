...
read_main_mirror_from_deb822_file() {
  ...
  while IFS= read -r line; do
    ...
    matches "$line" 'URIs:\s+([^ ]+)' && {
      mirror_uri=${BASH_REMATCH[1]}
      continue
    }
    matches "$line" 'Components:\s+. *(main)(\s+|$)' && {
      mirror_main=true
      continue
    }
    [[ -n $mirror_uri && $mirror_main == "true" ]] && {
      echo "$mirror_uri"
      return
    }
  done <"$1"
