...
shrink_img() {
  log "Shrinking image..."
  local p_out; p_out=$(parted -ms "$IMG" unit B print)
  local start; start=$(awk -F: 'END{print $2}' <<<"$p_out" | tr -d B)
  ...
  if ((sz_cur > sz_min)); then
    ...
    log "Image already minimal"; losetup -d "$LOOP"; LOOP=""
  fi
}

setup_tgt() {
  LOCK="/run/lock/rf2fs-${TGT//\//_}.lock"; mkdir -p "${LOCK%/*}"
  ...
  wipefs -af "$TGT"; parted -s "$TGT" mklabel msdos mkpart primary fat32 0% "$BOOT_SIZE" mkpart primary "$BOOT_SIZE" 100% set 1 boot on
  partprobe "$TGT"; udevadm settle; sleep 1
  [[ $TGT =~ nvme|mmcblk|loop ]] && local p="p" || local p=""
  BP="${TGT}${p}1"; RP="${TGT}${p}2"
  for i in {1..30}; do [[ -b $BP && -b $RP ]] && break; sleep 0.5; done
...
clone() {
  log "Cloning data..."
  ((DRY)) && return
  LOOP=$(losetup --show -f -P "$IMG"); MOUNTS=()
  [[ $LOOP =~ nvme|mmcblk|loop ]] && local p="p" || local p=""
  mkdir -p "$WD"/{s,t}/{b,r}
  mount -o ro "${LOOP}${p}1" "$WD/s/b"; MOUNTS+=("$WD/s/b")
  mount -o ro "${LOOP}${p}2" "$WD/s/r"; MOUNTS+=("$WD/s/r")
  mount "$BP" "$WD/t/b"; MOUNTS+=("$WD/t/b")
  mount "$RP" "$WD/t/r"; MOUNTS+=("$WD/t/r")
