...
assert_size(){
  ...
  [[ ! -b $dev ]] && return 0
  local img_bytes dev_bytes; img_bytes=$(stat -c%s "$img"); dev_bytes=$(blockdev --getsize64 "$dev")
  ((img_bytes > dev_bytes)) && die "Image ($((img_bytes/1024/1024))MB) exceeds target ($((dev_bytes/1024/1024))MB)."
}
select_target_interactive(){
  has fzf || die "fzf required for interactive selection."; log "Scanning for removable drives..."
  local selection; selection=$(lsblk -p -d -n -o NAME,MODEL,VENDOR,SIZE,TRAN,TYPE,HOTPLUG | \
    awk -v skip="${cfg[no_usb_check]}" 'tolower($0) ~ /disk/ && (skip == "1" || tolower($0) ~ /usb|mmc/)' | \
    fzf --header="TARGET SELECTION (Safety: USB/MMC Only)" --prompt="Select Drive > " --with-nth=1,2,3,4)
  [[ -z $selection ]] && die "No target selected."; echo "$selection" | awk '{print $1}'
}
# Utils
check_deps(){
  local -a deps=(losetup parted mkfs.f2fs mkfs.vfat rsync xz blkid partprobe lsblk flock awk curl) missing=()
  local cmd; for cmd in "${deps[@]}"; do has "$cmd" || missing+=("$cmd"); done
...
setup_target_device(){
  ...
  warn "${RED}WARNING: ALL DATA ON $TGT_PATH WILL BE ERASED!${DEF}"; wipefs -af "$TGT_PATH" &>/dev/null
  log "Partitioning..."; parted -s "$TGT_PATH" mklabel msdos
...
clone_data(){
  ...
  log "Syncing / (Rootfs)..."; rsync -aHAX --info=progress2 --exclude 'lost+found' "$WORKDIR/src/root/" "$WORKDIR/tgt/root/"; sync
}
...
EOF
  ((cfg[ssh])) && touch "$WORKDIR/tgt/boot/ssh"; log "Configuration complete."
}
...
[[ $EUID -ne 0 ]] && die "This script requires root privileges (sudo)."; check_deps
if [[ -z $SRC_PATH ]]; then
...
msg "${GRN}SUCCESS:${DEF} Flashed to $TGT_PATH with F2FS."; msg "You can now safely remove the device."
