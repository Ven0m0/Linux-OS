...
# --- Platform Detection ---
detect_platform(){
  if command -v pacman &>/dev/null>/dev/null; then
    echo "arch"
  elif command -v apt-get &>/dev/null>/dev/null; then
    echo "debian"
  else
    echo "unknown"
  fi
...
debloat_arch(){
  ...
  if pacman -Qq pkgstats &>/dev/null>/dev/null; then
    sudo pacman -Rcns --noconfirm pkgstats &>/dev/null>/dev/null || :
  fi
  ...
  # Disable UFW logging
  command -v ufw &>/dev/null>/dev/null && sudo ufw logging off 2>/dev/null || :
}
...
main(){
  ...
  *)
  ...
  echo -e "\nDebloat complete! Press any key to exit."
  read -n 1 -s
}
main "$@"
