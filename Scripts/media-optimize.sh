...
detect_video_codec() {
  ...
  if ffmpeg_has_encoder libsvtav1 || ffmpeg_has_encoder libaom-av1; then
    VIDEO_CODEC="av1"
  elif ffmpeg_has_encoder libvpx-vp9; then
    VIDEO_CODEC="vp9"
  elif ffmpeg_has_encoder libx265; then
    VIDEO_CODEC="h265"
  elif ffmpeg_has_encoder libx264; then
    VIDEO_CODEC="h264"
...
optimize_png() {
  ...
  [[ $success -eq 1 ]] && mv "$tmp" "$out" && echo "$((orig - $(get_size "$out")))" || {
    rm -f "$tmp"
    return 1
  }
}
...
optimize_image() {
  ...
  [[ $SKIP_EXISTING -eq 1 ]] && is_already_optimized "$src" && {
    ((STATS_SKIPPED++))
    return 0
  }
  ...
  [[ -f $out && $KEEP_ORIGINAL -eq 1 && $INPLACE -eq 0 ]] && {
    ((STATS_SKIPPED++))
    return 0
  }
  ...
  [[ $DRY_RUN -eq 1 ]] && {
    log "[DRY] Would process: $(basename "$src") → $fmt"
    return 0
  }
  ...
  if [[ $fmt != "$ext" ]]; then
    ...
    [[ $success -eq 1 ]] && mv "$tmp" "$out" || {
      warn "Conversion to $fmt failed"
      rm -f "$tmp"
      return 1
    }
  else
  ...
  if ((new > 0 && new < orig)); then
    ...
    ((MIN_SAVINGS > 0 && pct < MIN_SAVINGS)) && {
      warn "Savings ${pct}% < threshold ${MIN_SAVINGS}%"
      rm -f "$out"
      ((STATS_SKIPPED++))
      return 1
    }
...
optimize_video() {
  ...
  *) enc=(-c:v libvpx-vp9 -crf "$VIDEO_CRF" -b:v 0 -row-mt 1) ;;
  ...
  [[ $success -eq 0 ]] && {
    warn "Video optimization failed"
    return 1
  }
