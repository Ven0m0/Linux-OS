...
find_files(){
  ...
  local -n result="$2"
  # Common exclusions
  local exclude_dirs=(.git node_modules .cache .var .rustup .wine .zim .void-editor .vscode .claude Linux-Settings target dist build)
  if [[ -n $FD ]]; then
...
print_summary(){
  ...
  log ""
  # Unique counts
  local -i unique_modified=0 unique_errors=0
